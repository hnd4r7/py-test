...
app = Dash(
    __name__, external_stylesheets=[dbc.themes.BOOTSTRAP], background_callback_manager=background_callback_manager
)
...
modal = dbc.Modal(
    [
        ...
        dbc.ModalBody(html.Progress(id="progress_bar", value="0", style={"width": "100%"})),
        dbc.ModalFooter(dbc.Button("Cancel", id="cancel_button_id", className="ms-auto", n_clicks=0)),
    ],
    id="modal",
    is_open=False,
    backdrop="static",
...
@callback(
    ...
    background=True,
    running=[(Output("button_id", "disabled"), True, False), (Output("modal", "is_open"), True, False)],
